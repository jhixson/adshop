...
class View_Model extends Model {
	...
	public function search_items($term,$desc=false,$page=1) {
		if(!empty($term)) {
		  ...
		  $cars = $this->db->query("select id, title from subsubcategories where subcategory_id = 1");
		  $bikes = $this->db->query("select * from subsubcategories where subcategory_id = 2");
		  $dogs = $this->db->query("select * from subsubcategories where subcategory_id = 12");
		  
		  $subsubcats = array();
		  foreach($cars as $c) {
		    if(in_array(strtolower($c->title),$terms))
		      $subsubcats[] = $c->id;
		  }
		  foreach($bikes as $b) {
		    if(in_array(strtolower($b->title),$terms))
		      $subsubcats[] = $b->id;
		  }
		  foreach($dogs as $d) {
		    if(in_array(strtolower($d->title),$terms))
		      $subsubcats[] = $d->id;
		  }
